...
class Cates extends BaseAdmin{
    ...
    {
        $id = input('get.id');
        $data = $this->db->table('product_cates')->where(['id'=>$id])->item();
    ...
    public function delete()
    {
        $id = input('post.id');
        if($id == ''){
            exit(json_encode(['code'=>1,'msg'=>'非法操作']));
        }
        $res = $this->db->table('product_cates')->where(array('pid'=>$id))->item();
        if($res){
            exit(json_encode(['code'=>1,'msg'=>'请先删除子类']));
        }
        $del = $this->db->table('product_cates')->where(['id'=>$id])->delete();
        if($del){

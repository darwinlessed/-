...
class Home extends Base
{
	...
	public function index(){
		...
		# 获取商品
		$product = $this->db->table('product')->where(array('status'=>1,'hot'=>1))->limit(10)->lists();
		if($product){
        ...
        # 获取普通商品
        $normal_product = $this->db->table('product')->where(array('status'=>1))->limit(4)->lists();
        if($normal_product){
            foreach($normal_product as &$new_v){
                $new_imgs = explode(';', $new_v['img']);
                $new_img  = str_replace('\\','/',$new_imgs[0]);
                $new_v['img'] = 'http://'.$_SERVER['HTTP_HOST'].$new_img;
                $new_v['add_time_s'] = date('Y-m-d H:i',$new_v['add_time']);
		...
		# 获取一级分类
        $cates = $this->db->table('product_cates')->where(['pid'=>0])->limit(10)->lists();
		if($cates){
//		    dump($cates);
            foreach ($cates as $key=>$value){
                ...
                $value['add_time_s'] = date('Y-m-d H:i',$new_v['add_time']);
                $cates[$key] = $value;
            }
        }

        # 获取主推美女
        $hot =  $this->db->table('product')->where(['cid'=>44])->limit(2)->lists();
//        dump($hot);
        if($hot){
...
//                dump($imgs);
                foreach ($imgs as $img) {
                    $res = str_replace('\\','/',$img);
                    $hot[$key]['imgs'][] = 'http://'.$_SERVER['HTTP_HOST'].$res;
                }
            }
        }
    ...
    public function shop_one_list()
    {
        ...
        $product = $this->db->table('product')->order($orders)->where(array('status'=>1,'cid'=>$cid))->pages_api(4,$page);
        if($product['lists']){
	...
	public function shop_order(){
		$id = input('post.id');
		$uid = input('post.uid');
		$product_info = $this->db->table('cart')->where(array('product_id'=>$id,'uid'=>$uid))->item();
		$total_price = 0;
		$product = $this->db->table('product')->where(array('id'=>$id))->item();
		$img = explode(';', $product['img']);
		$product_img  = str_replace('\\','/',$img[0]);
		$product['imgs'] = 'http://'.$_SERVER['HTTP_HOST'].$product_img;
		$product['car']['count'] = $product_info['count']? $product_info['count'] : 1 ;
		$arr = array(
			'car' => array($product),
			'total_price' => sprintf("%.2f",$product['price']*$product['car']['count']),
			'product_id' => $id
		);
		$this->returnCode(0,$arr);
	}
	...
	public function order_list(){
		...
		$page = (int)input('post.page');
		$uid = (int)input('post.uid');
		$orders = $this->db->table('orders')->order('add_time DESC')->where(array('status'=>$status,'uid'=>$uid))->pages_api(4,$page);
//		dump($orders);
		if($orders['lists']){
